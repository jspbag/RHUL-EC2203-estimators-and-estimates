...
- An **estimator** of $\mu_Y$ is a function of the sample to be drawn at random from the population

  So, an estimator is a random variable with statistical properties that we can study
...
- Evaluate $\hat{\mu}_Y$ many ($\rightarrow \infty$) times on different size-$n$ i.i.d. samples, producing many estimates with average $\mathrm{E}(\hat{\mu}_{Y})$
